...
──────────────────
  ...
  ─────────────────    ─────────────────────────────────────────────  ─────────
  qwen2vl-2b           Qwen/Qwen2-VL-2B-Instruct                      ~3.5 GB
  qwen2vl-7b           Qwen/Qwen2-VL-7B-Instruct                      ~8  GB
  qwen25vl-3b          Qwen/Qwen2.5-VL-3B-Instruct                    ~4  GB
  qwen25vl-7b          Qwen/Qwen2.5-VL-7B-Instruct                    ~8  GB  ← default
...
if DEVICE == "cuda":
    # Use bfloat16 for modern GPUs (Ampere+), float16 for older, fp32 for CPU
    DTYPE = torch.bfloat16 if torch.cuda.is_bf16_supported() else torch.float16
    print(f"  [device] CUDA detected. Running on GPU ({DTYPE}).")
else:
    DTYPE = torch.float32
    print("  [device] CUDA not found. Running on CPU (float32).")
# ══════════════════════════════════════════════════════════════════════════════
...
@dataclass
class LoanFormData:
    """Structured data extracted from an Egyptian Loan Application Form."""

    # --- Section 1: Client Data (بيانات العميل) ---
    loan_limit:             Optional[str] = None  # قيمة القرض المسموح بها (مثلاً: من ألف حتى ٥٠ ألف جـ)
    client_name:            Optional[str] = None  # اسم العميل
    birth_date:             Optional[str] = None  # تاريخ الميلاد
    marital_status:         Optional[str] = None  # الحالة الاجتماعية (عزب / متزوج / مطلق)
    dependents_count:       Optional[str] = None  # عدد من يمول
    education_level:        Optional[str] = None  # المستوى التعليمي
    client_address:         Optional[str] = None  # عنوان العميل
    residence_type:         Optional[str] = None  # نوع السكن (تمليك / إيجار)
    residence_start_date:   Optional[str] = None  # تاريخ بداية الإقامة
    home_phone:             Optional[str] = None  # تليفون المنزل
    mobile_phone:           Optional[str] = None  # تليفون المحمول
    monthly_income:         Optional[str] = None  # متوسط الدخل الشهري من النشاط
    other_income:           Optional[str] = None  # دخل من مصادر أخرى
    other_income_source:    Optional[str] = None  # نوع المصدر
    monthly_operations:     Optional[str] = None  # متوسط العمليات الشهرية
    monthly_expenses:       Optional[str] = None  # متوسط المصروفات الشهرية
    bank_transactions:      Optional[str] = None  # معاملات بنكية / جمعيات (نعم / لا)
    bank_name:              Optional[str] = None  # اسم البنك / الجمعية

    # --- Section 2: Facility Data (بيانات المنشأة) ---
    facility_name:          Optional[str] = None  # اسم المنشأة
    facility_address:       Optional[str] = None  # عنوان المنشأة
    facility_place_type:    Optional[str] = None  # مكان المنشأة (تمليك / إيجار)
    activity_type:          Optional[str] = None  # نوع النشاط
    activity_duration:      Optional[str] = None  # مدة النشاط

    # --- Section 3: Guarantee Data (بيانات الضمان) ---
    guarantor_name:         Optional[str] = None  # اسم الضامن
    guarantor_address:      Optional[str] = None  # عنوان الضمان
    guarantee_type:         Optional[str] = None  # نوع الضمان

    # --- Section 4: Requested Loan Data (بيانات القرض مطلوب) ---
    requested_loan_amount:  Optional[str] = None  # قيمة القرض المقترح
    repayment_period:       Optional[str] = None  # مدة السداد

    # --- Section 5: Client Acknowledgment (إقرار العميل) ---
    ack_client_name:        Optional[str] = None  # اسم العميل في الإقرار
    ack_client_address:     Optional[str] = None  # عنوان العميل في الإقرار
    ack_client_national_id: Optional[str] = None  # رقم قومي العميل في الإقرار
    ack_client_branch:      Optional[str] = None  # فرع البنك في الإقرار

    # --- Section 6: Guarantor Acknowledgment (إقرار الضامن) ---
    ack_guarantor_name:        Optional[str] = None  # اسم الضامن في الإقرار
    ack_guarantor_address:     Optional[str] = None  # عنوان الضامن في الإقرار
    ack_guarantor_national_id: Optional[str] = None  # رقم قومي الضامن في الإقرار
    ack_guarantor_branch:      Optional[str] = None  # فرع البنك في الإقرار
...
# ── 2d. Loan Form prompts ──────────────────────────────────────────────────────

RAW_OCR_PROMPT_LOAN = (
    "هذا نموذج طلب قرض بنكي مصري. "
    "استخرج النصوص بجوار كل خانة (في النقط ..... أو بعد علامة /). "
    "انسخ الكلمات العربية والأرقام تماماً كما هي، بدون شرح."
    "--- ENGLISH ---\n"
    "This is an Egyptian Bank Loan Application form. "
    "Transcribe all text next to each field (within dots ..... or after /). "
    "Copy Arabic words and numbers exactly as printed. Output RAW TEXT ONLY."
)


def build_parse_prompt_loan(raw_text: str) -> str:
    return f"""
Act as an Expert Document OCR Extractor for Egyptian Bank Forms.
Analyze the raw OCR text below:
\"\"\"
{raw_text}
\"\"\"

Extract the values next to these label keys. If a value is "....." or empty, return null.

1. Client Data:
   - loan_limit: (قرض من ... حتى)
   - client_name: (اسم العميل)
   - birth_date: (تاريخ الميلاد)
   - marital_status: (الحالة الاجتماعية)
   - dependents_count: (عدد من يمول)
   - education_level: (المستوى التعليمي)
   - client_address: (عنوان العميل)
   - residence_type: (نوع السكن)
   - residence_start_date: (تاريخ بداية الإقامة)
   - monthly_income: (متوسط الدخل الشهري من النشاط)
   - other_income: (دخل من مصادر أخرى)
   - bank_name: (اسم البنك/الجمعية)
...
3. Requested Loan:
   - requested_loan_amount: (قيمة القرض المقترح)
   - repayment_period: (مدة السداد)

Rules:
1. Verbatim Arabic for names/addresses.
2. Convert Eastern Arabic digits (٠-٩) to Western (0-9).
3. Use null for empty/unfilled fields.
...
def _regex_extract_loan(raw: str) -> dict:
    """Rule-based extraction for Egyptian Loan Application Form."""
    result: dict = {}
    text = _to_western(raw)

    def extract_after(label: str, text: str, next_labels: List[str]) -> Optional[str]:
        pattern = rf"{label}\s*[:/]\s*(.*?)(?=\s*(?:{'|'.join(next_labels)}|$))"
    ...
    # Section mapping
    result['client_name'] = extract_after("اسم العميل", text, ["تاريخ الميلاد", "الحالة"])
    result['loan_limit'] = extract_after("قرض", text, ["طلب"])
    result['requested_loan_amount'] = extract_after("قيمة القرض المقترح", text, ["مدة"])
    result['facility_name'] = extract_after("اسم المنشأة", text, ["عنوان"])
    
    # Specific fix for common OCR errors
    if "صليح" in raw and "نوع السكن" in raw:
        result['residence_type'] = "تمليك"
...
def _run_donut(image_path: str, prompt: str) -> str:
    ...
    if _CACHE.backend != MODEL_ID:
        ...
        _CACHE.model     = VisionEncoderDecoderModel.from_pretrained(MODEL_ID).to(DEVICE)
        _CACHE.model.eval()
        _CACHE.backend   = MODEL_ID
...
def _run_qari(image_path: str, prompt: str) -> str:
    ...
    text_in = _CACHE.processor.apply_chat_template(
        messages, tokenize=False, add_generation_prompt=True)
    inputs = _CACHE.processor(text=text_in, images=[image], return_tensors="pt").to(DEVICE)
...
def _run_lighton_ocr(image_path: str, prompt: str) -> str:
    ...
    # Ensure proper dtype for inputs
...
def _process_image(image_path: str, side: str,
    ...
    elif doc_type == "loan_form":
        ocr_prompt = RAW_OCR_PROMPT_LOAN
    else:
    ...
    if doc_type == "driver_license":
        ...
        parse_prompt = build_parse_prompt_loan(raw)
    else:
    ...
    if doc_type == "driver_license":
        ...
        # Broad mapping for flexible loan forms
        cleaned = {k: _clean(v) for k, v in parsed.items()}
        regex_fn = _regex_extract_loan
...
class EgyptianIDExtractor:
    ...
    Usage:
        extractor = EgyptianIDExtractor(backend="qwen25vl-7b")
        result = extractor.extract(front_image="front.jpg", back_image="back.jpg")
        print(result.to_json())
    """

    def __init__(self,
                 backend: str  = "qwen25vl-7b",
                 verbose: bool = False):
...
class DriverLicenseExtractor:
    ...
    Usage:
        extractor = DriverLicenseExtractor(backend="qwen25vl-7b")
...
class PassportExtractor:
    ...
    Usage:
        extractor = PassportExtractor(backend="qwen25vl-7b")
...
class LoanFormExtractor:
    """
    Extract structured data from an Egyptian Bank Loan Application form.

    Usage:
        extractor = LoanFormExtractor(backend="qwen25vl-7b")
        result = extractor.extract(image="loan_form.jpg")
        print(result.to_json())
    """
    ...
    def extract(self, image: str) -> LoanFormData:
        if not image:
            raise ValueError("Provide an image path")

        run_fn = self.backend_cfg["fn"]
        result = LoanFormData(backend_used=self.backend_cfg["name"])
        ...
        # Confidence
        key_fields = ["client_name", "requested_loan_amount", "facility_name"]
        filled = sum(1 for f in key_fields if getattr(result, f))
...
Available backends (use --list-backends for full table):
  qwen25vl-7b  ← default, ~8 GB RAM, latest Qwen vision model
  qwen2vl-2b   — lightweight option, ~3.5 GB RAM, recommended for CPU
...
Examples:
  ...
  python extractor.py --doc-type driver_license --image license.jpg --verbose
    ...
    parser.add_argument("--doc-type", default="national_id",
                        choices=["national_id", "driver_license", "passport", "loan_form"],
    ...
    parser.add_argument("--backend", default="qwen25vl-7b",
                        choices=list(BACKENDS),
                        help="Vision model backend (default: qwen25vl-7b)")
    ...
    if doc_type == "driver_license":
        ...
        if not args.image:
            parser.error("--image is required for loan_form")
        extractor = LoanFormExtractor(backend=args.backend, verbose=args.verbose)
        result    = extractor.extract(image=args.image)
        raw_data  = {"raw_text_front": result.raw_text_front}
    else:
